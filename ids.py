...
def get_invader_list():
    outlist = ["None"]
    for config_name in INVADER_CONFIGURATIONS:
        config = INVADER_CONFIGURATIONS[config_name]
        if len(config) == 0:
            outlist.append("Invalid Invader Slot")
            continue
        if config[0]['levelSpread'] == 0:
            outstr = "Lv."+str(config[0]['level'])+" "+config[0]['monsterId'] + " "+ str(config[0]['probability1'])+"%"
            for monster in config[1:]:
                outstr += "  | Lv."+str(monster['level'])+" "+monster['monsterId'] + " " + str(monster['probability1'])+"%"
            outlist.append(outstr)
        elif config[0]['levelSpread'] == 2:
            outstr = "Lv."+str(config[0]['level'])+"+/-1 "+config[0]['monsterId'] + " "+ str(config[0]['probability1'])+"%"
            for monster in config[1:]:
                outstr += "  | Lv."+str(monster['level'])+"+/-1 "+monster['monsterId'] + " " + str(monster['probability1'])+"%"
            outlist.append(outstr)
        else:
            outstr = "Lv."+str(config[0]['level'])+"+/-2 "+config[0]['monsterId'] + " "+ str(config[0]['probability1'])+"%"
            for monster in config[1:]:
                outstr += "  | Lv."+str(monster['level'])+"+/-2 "+monster['monsterId'] + " " + str(monster['probability1'])+"%"
            outlist.append(outstr)
    return outlist
...
class Monster(Enum):
    ...
    altaroth = 33
    kelbi = 34
    giggi_egg = 35  # Gigginox Egg
    bnahabra_blue = 36  # Bnahabra (Blue Wing)
    bnahabra_orange = 37  # Bnahabra (Orange Wing)
    bnahabra_green = 38  # Bnahabra (Green Wing)
    bnahabra_red = 39  # Bnahabra (Red Wing)
    rock = 40  # ---- INVALID AS AN IN-QUEST MONSTER ---
